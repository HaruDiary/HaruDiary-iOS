...
| 검사 | 실제 범위 | 포함하지 않는 것 |
|---|---|---|
| Logic tests | 실제 `DataModel.swift`의 Codable 필드 호환, 날짜 offset, 삭제/사진 정보 보존, 생성 기본값, 잘못된 입력 거부: 6개 XCTest | Firestore SDK의 Codable 동작, 서버 읽기/쓰기, 로그인, 월간 규칙 |
...
Domain을 분리한 뒤에는 이 타깃에 실제 Domain 파일을 연결하고 기존 중복 컴파일 연결을 정리한다.
